...
なお、バージョン番号のあとに-urlがついているのは、コード表を識別するsystem値の記法に"urn:oid:1.2.395...."形式ではなく、"http://..."形式を使用しているパッケージであることを示す。
  ...
  - jpfhir-terminology.r4　パッケージ
    ...
    - tgz形式 : [https://jpfhir.jp/fhir/core/terminology/jpfhir-terminology.r4-1.2.5.tgz](https://jpfhir.jp/fhir/core/terminology/jpfhir-terminology.r4-1.2.5.tgz)
 　 
...
```  bash
 java -jar [prog]/validator_cli_6.1.8.jar \
    [targets]/*.json \
      ...
      -ig [pkgClins]/jpfhir-terminology.r4-1.2.5.tgz  \
...
Validationコマンドのパラメータ説明
  ...
  - -tx n/a ：　外部のTerminologyServer を参照しないよう設定するオプション。ここでの手順では、パッケージ [jpfhir-terminology.r4-1.2.5]をロードしてローカルに配置しているので、外部のTerminologyServerへの参照は必要がない。
  - -ig [pkgClins]/jp-core.r4-1.1.2-clins.tgz : jp-core.r4 v1.1.2-url のパッケージ。必須。これがないとjp-coreを参照する際にエラーになる。
  - -ig [pkgClins]/jpfhir-terminology.r4-1.2.5.tgz ： jp-core.r4、jp-clinsから参照されるterminologyのパッケージ。必須。これがないと日本版CodeSystemやValueSetを参照する際にエラーになる。このパッケージには、JLAC10、医薬品マスター、標準病名マスター、ICD10分類コード表なども含まれるので、定期的に適切なバージョンへのアプデートが必要である。
...
Loading
  ...
  Load pkgClins/jpfhir-terminology.r4-1.2.5.tgz - 175 resources (00:03.988)
