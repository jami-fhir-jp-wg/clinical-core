...
# （３文書６情報を含む）電子カルテ情報共有サービスのためのFHIR実装ガイド
...
```
[用語説明]　FHIRリソース：　FHIRでは医療情報は、リソースというひとまとまりのデータ項目を集めた構造的なデータとして記述される。プログラミング言語でいう構造体のようなもので、メソッドのないClassのような概念に相当するともいえる。患者基本情報のPatientリソース、検査結果のObservationリソースなど、100種類以上のリソースが定義されている。それぞれのリソースの構造（どういうデータタイプのどんな要素から構成されているか）を定義しているのがProfile（プロファイル）である。Profile自体もひとつのリソースである（StructureDefinitionリソース）。リソースの種類のことをリソースタイプと呼ぶ。
```
```
...
　<br>
アレルギー情報と薬剤禁忌情報と、別に説明するように、同じプロファイルにしたがって記述され、コードやカテゴリーの情報にもとづいて受信側で区別される。また検査結果情報と感染症情報についても同様である。
<br>処方依頼情報には当面、注射点滴手法（自己注射を除く）によるものは含まれない。
<br>

* 上記のリソースから参照される関連情報
　以下の情報は上記主要の6情報から、その要素情報として参照される情報種別がいくつかある。
　これらは後述するContainedリソースとよばれる記述形態で、６情報のなかに埋め込み記述される。
  * [患者基本情報	Patientリソース][JP_Patient]
  * [外来受診・入院歴情報	Encounterリソース][JP_Encounter]
  * [医療者情報　Practitionerリソース][JP_Practitione]
  * [サービス依頼（オーダ）情報　ServiceRequestリソース][ServiceRequest]
    - 注）JP_ServiceRequestはJP-Coreで未定義なので、HL7FHIR基底プロファイルServiceRequestを参照のこと。<br>
  * [検査検体情報	Sepecimenリソース][JP_Specimen]  
  <br>

- 診療情報提供書、退院時サマリーに含まれる上記以外の情報種別の記載と取り扱い
  診療情報提供書、退院時サマリーの仕様書に記載されている上記以外の情報種別のリソースは、仕様書に従い記述して構わない。<br>
  「電子カルテ情報共有サービス」では、診療情報提供書、退院時サマリーは文書として取り扱われる以外に、これらの文書中に記述されている６情報はとりだされ、６情報として送信されたのと同様に管理される。しかし、文書中に記載されている６情報以外の情報種別のリソースは抽出されない。
