...
すなわち、
  - ＜IdentifierタイプA＞では、その医療機関のそのシステム系統では、過去未来に跨がり、すべてのリソース・インスタンスをひとつひとつ、必ず一意に、同じものとして識別できるID値をvalueに設定しなければならない。
...
### 電子カルテ情報共有サービス（仮称）での処理内容：
  前提として、以下で扱うidentifier.system値には６情報送信仕様で定められた固定値が設定されなければならない。固定値には２タイプあり、＜IdentifierタイプA＞と＜IdentifierタイプB＞に対応して、それぞれ異なる固定値が定められている。以下では＜IdentifierタイプA＞に対応するsystem固定値を"system-typeA"、＜IdentifierタイプB＞に対応するsystem固定値を"system-typeB"と記載する。
