...
すなわち、
  - ＜IdentifierタイプA＞では、その医療機関のそのシステム系統では、過去未来に跨がり、すべてのリソース・インスタンスをひとつひとつ、必ず一意に、同じものとして識別できるID値をvalueに設定<span style="color: red; ">しなければならない。</span>
  - ＜IdentifierタイプB＞では、その医療機関のそのシステム系統では、過去未来に跨がり、リソース・インスタンスが含まれるオーダ単位や結果報告書単位で、それらのオーダ単位や結果報告書が必ず一

このルールの意味を理解するために、受信側のサービスの処理を先に説明する。

### 電子カルテ情報共有サービス（仮称）での処理内容：
  前提として、以下で扱うidentifier.system値には６情報送信仕様で定められた固定値が設定<span style="color: red; ">されなければならない。</span>固定値には２タイプあり、＜IdentifierタイプA＞と＜IdentifierタイプB＞に対応して、それぞれ異なる固定値が定められている。以下では＜IdentifierタイプA＞に対応するsystem固定値を"system-typeA"、＜IdentifierタイプB＞に対応するsystem固定値を"system-typeB"と記載する。
