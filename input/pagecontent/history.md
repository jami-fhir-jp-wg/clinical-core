...
### ２文書５情報＋患者サマリー（CLINS）  Ver. 1.7.0  (2024.12.19) バージョン番号変更せず
  - ダウンロードパッケージが格納されているフォルダを移動した。
  - 過去バージョンのIGとダウンロードパッケージも全バージョン一覧メニューに掲載した。
  - validation方法の説明文を上記に対応する内容を追加し、複数のバージョンを同時にValidationしたい場合の方法についても記載した。
...
### ２文書５情報＋患者サマリー（CLINS）  Ver. 1.7.0  (2024.12.17)
  ...
  - ValueSet:Dダイマー（DD-定性）のJLAC10,JLAC11に対応するValueSetが不足していたため追加。
