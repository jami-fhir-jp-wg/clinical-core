...
* identifier[resourceIdentifier].system ^comment = "このアレルギー情報を作成した施設内で、このアレルギー情報を他のアレルギー情報と一意に区別できるIDを発番できる場合にのみ、このsystem値（$JP_ResourceInstanceIdentifier）を使用すること。1回の登録で複数のアレルギー情報が登録される場合に、それらは同一のIDで登録される場合でもこのsystem値を使用する。"
...
* code  ^comment = "使用するコード表（コード化する場合）：ValueSethttp://jpfhir.jp/fhir/core/ValueSet/JP_AllergyIntolerance_VS
...
* patient ^comment = "ContainedリソースによりPatientリソースを本リソースの要素として記述した上で、そのリソースをLiteral 参照する方法(Patient.idを#で記述する)をとっても構わない。"
