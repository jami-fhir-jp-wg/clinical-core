...
Description: "Encounterリソース（受診時・入院時等のEncounter情報）プロファイル"
...
* . ^short = "ー"
* . ^definition = "診療情報提供では、紹介理由を記述する受診時情報. 診療サマリーではサマリー対象となる受診や入院に関する受診時または入院時情報。診療５情報において埋め込みリソースとして記述する場合にも適用できる。"
