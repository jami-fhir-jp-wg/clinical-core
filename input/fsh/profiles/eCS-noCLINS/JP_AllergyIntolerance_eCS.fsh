...
Description: "AllergyIntoleranceリソース（アレルギー情報／薬剤アレルギー等）プロファイル"
...
//* obeys meta-tag-code-LTS-restriction
...
* meta.profile 1.. MS
  * insert relative_short_definition("本プロファイルを識別するURLとバージョンを指定する。http://jpfhir.jp/fhir/eCS/StructureDefinition/JP_AllergyIntolerance_eCS を設定する。バージョン指定を省略した場合には、データ受信時点の最新バージョンとみなされる。")
...
* contained[encounter] only  JP_Encounter_eCS
  ...
  * ^comment = "encounter要素から参照される場合には、そのJP_Encounter_eCSに従うリソースの実体。JP_Encounter_eCSに従うリソースにおける必要最小限の要素だけが含まれればよい。ここで埋め込まれるJP_Encounter_eCSに従うリソースでは、Encounter.classにこの情報を記録したときの受診情報（入外区分など）を記述して使用する。"
