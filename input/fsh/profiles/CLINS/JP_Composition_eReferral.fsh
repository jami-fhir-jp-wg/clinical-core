...
* section[{sectionSlice01}].code.coding from http://jpfhir.jp/fhir/{documentCategory}/ValueSet/document-section
...
RuleSet: compositionSectionTitleCodeDefinition(compositionSection,documentCategory,sectionSlice02,sectionName,sectionCode)
...
* section[{compositionSection}].section[{sectionSlice02}].code.coding from http://jpfhir.jp/fhir/{documentCategory}/ValueSet/document-section
...
* section[compositionSection].section[infectiousDiseaseInformationSection].entry only Reference(JP_Observation_Common or JP_Condition_eCS)
...
* section[compositionSection].section[infectiousDiseaseInformationSection].entry ^definition = "感染症情報を記述して参照する。
                                                                1つの感染症情報につき1つのObservationリソース(JP_Observation_Commonの派生プロファイル（たとえばJP_Observation_LabResult_eCS）に従うリソース）またはConditionリソース（JP_Condition_eCS）で記述されたものを参照する。
