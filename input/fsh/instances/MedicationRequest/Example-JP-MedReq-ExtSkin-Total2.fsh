...
Usage: #example
Description: "【注意喚起例】：入院処方　外用（塗布）全量処方　1回量をコードで記述できない例. ただし医薬品コードを「標準コード（YJまたは規格別成分コード）」で記述していないために電子カルテ情報共有サービスに送信する場合にはエラー（警告）になる例。"
* note.text = "【注意喚起例】：入院処方　外用（塗布）全量処方　1回量をコードで記述できない例. ただし医薬品コードを「標準コード（YJまたは規格別成分コード）」で記述していないために電子カルテ情報共有サービスに送信する場合にはエラー（警告）になる例。"
...
* identifier[requestIdentifierCommon].value = "1311234567-9990767-0A172C13361BBE2021070517301001-1"
* identifier[requestIdentifier].system = $JP_ResourceInstanceIdentifier
* identifier[requestIdentifier].value = "1311234567-9990767-0A172C13361BBE2021070517301001-1"
* status = #completed
* intent = #order
