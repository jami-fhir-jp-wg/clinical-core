...
Usage: #example
Description: "【意図的エラー例】：入院処方　外用（塗布）全量処方　1回量をコードで記述できない例. ただし医薬品コードを「標準コード（YJまたは規格別成分コード）」で記述していないために電子カルテ情報共有サービスに送信する場合にはエラー（警告）になる例。"
* note.text = "【意図的エラー例】：入院処方　外用（塗布）全量処方　1回量をコードで記述できない例. ただし医薬品コードを「標準コード（YJまたは規格別成分コード）」で記述していないために電子カルテ情報共有サービスに送信する場合にはエラー（警告）になる例。"
