...
* human = "Bundle-entry-{par1}: Bundleのentryとして含まれる{par1}は、プロファイル{par2}のいずれかのバージョンに準拠していなければならない。"
* severity = #error 
* expression = "(entry.resource.ofType({par1}).exists().not()) or ((entry.resource.ofType({par1}).meta.profile.where(($this.indexOf('|')>0 and ($this.substring(0,$this.indexOf('|'))!='{par3}'))).exists()).not() ) and ((entry.resource.ofType({par1}).meta.profile.where(($this.indexOf('|')<=0 and ($this!='{par3}'))).exists()).not() )" 
